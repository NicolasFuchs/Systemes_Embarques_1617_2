...
{
	uint32_t period = f>0 ? SYSCLK/f : 0;
	uint32_t index = 0;

	while((index < ARRAY_SIZE(dividers)) && (period / dividers[index].div >= TB_PRD_MAX))index++;

	struct divider res = {
			.prd = f>0 ? SYSCLK/f/dividers[index].div : 0,
			.clkdiv = dividers[index].clkdiv,
			.hsdiv = dividers[index].hsdiv,
	};
...
void epwm1_init()
{
	...
	// as recommanded by teacher (tp.06-mini-projet-thermo_buzzer-handout.pdf)
...
void epwm1_set_frequency(uint32_t freq)
{
	...
	uint16_t reset_tbctl = ((1 << 16) -1) - TB_SYNC_DISABLE - (3<<TB_HSDIV_SHIFT) - (3<<TB_CLKDIV_SHIFT);

	struct divider div = get_divider(freq);

	pwm->epwm.tbprd = div.prd;
	pwm->epwm.tbcnt = 0;
	pwm->epwm.tbctl &= reset_tbctl;
