...
static const char* l2s[] = {"t-high", "t-low"};

static thermo_alarm_eh_t alarm_eh;

static void thermo_alarm_eh(enum am335x_gpio_modules module, uint32_t pin, void* param)
{
	(void)param;
	bool is_alarm_on = !am335x_gpio_get_state (module, pin);
	alarm_eh(is_alarm_on);
}


void thermo_init(thermo_alarm_eh_t routine)
{
	alarm_eh = routine;

	am335x_i2c_init(AM335X_I2C2, 400000);

	am335x_gpio_attach (ALARM_GPIO, ALARM_PIN, AM335X_GPIO_IRQ_BOTH, true, 
			    thermo_alarm_eh, 0);
}
