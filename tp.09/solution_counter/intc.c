 ...
 * routine.
 */
void intc_irq_isr(enum interrupt_vectors int_vector, void* addr, void* param)
{
	(void)int_vector; (void)addr; (void)param; // unused arguments

	// get IRQ interrupt vector from INTC and extract the vector
	...
	if (handlers[vector].routine != 0) {
		...
		if (handlers[INTC_SPURIOUS].routine != 0)
			handlers[INTC_SPURIOUS].routine 
				(vector, handlers[INTC_SPURIOUS].param);
	}

	// enable processing of subsequent pending interrupts 
	intc->control = 0x3;
}
...
void intc_init()
{
	...
	for (int i=ARRAY_OF(intc->flags)-1; i>=0; i--) {
		intc->flags[i].mir = -1;
		intc->flags[i].mir_set = -1;
		intc->flags[i].isr_clear = -1;
	}
	// 
	for (int i=ARRAY_OF(intc->ilr)-1; i>=0; i--) {
		intc->ilr[i] = 0;
	}
...
{
	if ((vector != INTC_SPURIOUS) && (vector < INTC_NB_OF_VECTORS)) {
		if (force) 
